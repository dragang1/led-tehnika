...
import { getCategorySlug } from './cms/utils.js';
...
export function generateProductSchema(product, categoryName, categorySlug, baseDomain) {
  ...
  let imageUrl = product.image?.[0] ? getImageUrl(product.image[0]) : `${baseDomain}/logo-black.png`;

  const images = product.image && Array.isArray(product.image)
    ? product.image.map((img) => getImageUrl(img)).filter((url) => url && url !== '/placeholder.png')
    : [imageUrl];
...
export function generateBlogPostingSchema(post, slug, baseDomain) {
  const description = post.content
    ? post.content.slice(0, 155).replace(/#/g, '').replace(/\n/g, ' ').trim()
    : post.title || '';
  const datePublished = post.date || new Date().toISOString().split('T')[0];
  const url = `${baseDomain}/blog/${slug}`;
  const logoUrl = `${baseDomain}/logo-black.png`;
...
// Generate ItemList schema for product listing pages (e.g. /proizvodi)
export function generateItemListSchema(products, baseDomain, limit = 50) {
