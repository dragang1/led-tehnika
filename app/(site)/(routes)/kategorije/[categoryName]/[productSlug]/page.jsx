import ProductDetailPage from '@/app/(site)/(routes)/kategorije/_components/ProductDetailPage';
...
export async function generateMetadata({ params }) {
  ...
  try {
    const product = await CmsApi.getProductBySlug(productSlug);
    if (!product) {
      return {
    ...
    const imageUrl = getImageUrl(product.image?.[0]) || `${baseDomain}/logo-black.png`;
