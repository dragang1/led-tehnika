...
const ProductDetailPage = ({ product, categoryName, categorySlug }) => {
  ...
  const selectedImageInitial = productImages?.[0] ? resolveImageUrl(productImages[0]) : '/placeholder.png';
  ...
  useEffect(() => {
    const fetchRelated = async () => {
      ...
      try {
        const related = await CmsApi.getProductsByCategory(categoryName, productId);
        ...
        <div className="mt-12 px-2 sm:px-4 max-w-screen-xl mx-auto">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 sm:gap-6">
            {relatedProducts.map((item) => {
              ...
              const relatedCategorySlug = getCategorySlug(relatedCategory) || 'nepoznata-kategorija';
              ...
              <Link key={item.id} href={`/kategorije/${relatedCategorySlug}/${item.slug}`}>
                ...
                >
                  <div className="relative w-full aspect-square bg-gradient-to-br from-gray-50 to-gray-100 overflow-hidden">
                    <Image 
                      src={resolveImageUrl(item.image?.[0])} 
