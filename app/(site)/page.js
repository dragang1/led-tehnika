...
export async function generateMetadata() {
  ...
  try {
    const featured = await CmsApi.getProductBySlug('motor-za-kapiju-set');
    if (featured?.name) {
...
export default async function Home() {
  const [categoryList, productList, featuredProductRaw, slidersResponse] = await Promise.all([
    ...
    CmsApi.getAllProducts(),
    CmsApi.getProductBySlug('motor-za-kapiju-set'),
    CmsApi.getSliders(),
  ]);
  ...
  if (featuredProductRaw?.image?.[0]?.url) {
    ...
    const imageUrl = getImageUrl(featuredProductRaw.image[0]);
    if (imageUrl) {
